...
export const seatChange = (
  ...
  return async (dispatch, getState) => {
    const actionType =
      status === TicketStatus.Unsold ? "holdTicket" : "unholdTicket"
    ...
    case "setTickets":
      return {
        ...
        otherTickets: action.tickets.filter(
          (ticket) => ticket.status === TicketStatus.Purchased
        ),
        myTickets: action.tickets.filter(
          (ticket) => ticket.status === TicketStatus.Held
        ),
      }
